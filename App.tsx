...
/**
 * Reusable component for vertical text display (e.g. table headers)
 */
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    document.body.className = darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900 transition-colors duration-300';
  }, [darkMode]);
  ...
  const handleTargetWeightConfirm = (customTarget?: number) => {
    ...
    if (typeof customTarget !== 'number') {
        if (isNaN(target) || target < range.min || target > range.max) {
            alert(`Abweichung zu hoch! Bitte zwischen ${Math.round(range.min)}g und ${Math.round(range.max)}g.`);
            return;
        }
    }

    const newRound: Round = { targetWeight: target, results: {} };
  ...
  const handleFinalTargetsConfirm = () => {
    ...
    if (!allFilled) { alert("Bitte alle individuellen Leergewichte eintragen."); return; }
    
    const finalRound: Round = { targetWeight: 0, isFinal: true, individualTargets: {}, results: {} };
    activePlayers.forEach(p => { finalRound.individualTargets![p.id] = parseInt(currentRoundTargets[p.id]); });
    ...
    <div className="min-h-screen flex flex-col p-4 md:p-8 overflow-x-hidden">
      <header className="flex justify-between items-center mb-8 w-full max-w-6xl mx-auto">
        ...
        <div className="flex space-x-2">
          <button onClick={() => setDarkMode(!darkMode)} className={`p-3 rounded-full shadow-md border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-100 border-gray-200'}`} aria-label="Toggle Theme">
            <i className={`fas ${darkMode ? 'fa-sun text-yellow-400' : 'fa-moon text-indigo-600'} text-xl`}></i>
          </button>
          {gameState !== GameState.START && (
            <button onClick={() => setShowResetConfirm(true)} className={`p-3 rounded-full shadow-md border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-100 border-gray-200'}`} aria-label="Exit Game">
      ...
      <main className="flex-1 flex flex-col items-center justify-center max-w-4xl w-full mx-auto relative mb-12">
        {gameState === GameState.START && (
          <div className="text-center animate-in fade-in duration-700 max-w-2xl w-full px-4">
            <div className="mb-12 flex justify-center">
              <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" className="block transition-transform hover:scale-105 active:scale-95">
                <img src={LOGO_URL} alt="1. Bundeswiega" className="w-64 h-64 md:w-80 md:h-80 lg:w-[400px] lg:h-[400px] object-contain drop-shadow-2xl" />
              </a>
            </div>
            <h2 className="text-xl md:text-2xl font-black mb-12 opacity-80 uppercase tracking-widest text-center">Das ultimative Wiegen-Spiel</h2>
            <div className="flex flex-col space-y-4 w-full max-w-sm mx-auto">
              ...
              <div className="flex flex-col space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setShowComingSoonModal(true)} className={`font-bold py-4 rounded-2xl shadow-md border transition-all ${darkMode ? 'bg-gray-800 border-gray-700 hover:bg-gray-700' : 'bg-gray-100 border-gray-200 hover:bg-gray-200'}`}>
        ...
        {gameState === GameState.PLAYER_COUNT && (
          <div className={`p-8 rounded-3xl shadow-2xl w-full max-w-md border animate-in slide-in-from-bottom-4 duration-300 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
        ...
        {(gameState === GameState.GAMEPLAY || gameState === GameState.FINAL_ROUND_RESULTS) && (
          <div className="w-full flex flex-col space-y-6 animate-in fade-in duration-500">
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {rounds.map((r, rIdx) => {
                        ...
                        const targetValue = (r.isFinal && r.individualTargets) ? 'IND' : `${r.targetWeight}g`;
                        return (
            ...
            <div className={`p-6 rounded-3xl border shadow-lg ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                <h3 className="font-bold mb-4 text-center uppercase text-sm tracking-widest opacity-60">Ergebnisse eintragen</h3>
                ...
                <button 
                  ...
                  style={{ backgroundColor: gameState === GameState.GAMEPLAY ? BRAND_COLOR : '#f59e0b' }}
        ...
        {gameState === GameState.RESULT_SCREEN && (
          <div className="w-full flex flex-col space-y-4 animate-in fade-in duration-500 max-h-screen overflow-y-auto px-1 pb-10 text-center">
            <h2 className="text-3xl font-black uppercase tracking-tighter mx-auto mb-6" style={{ color: BRAND_COLOR }}>🏆 Gesamtergebnis</h2>
            ...
            <div className="flex flex-col space-y-3 pt-6 px-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button 
                  ...
                  style={{ backgroundColor: BRAND_COLOR }}
                >
                  <i className="fas fa-camera text-lg"></i><span>Ergebnis-Foto</span>
                </button>
                <button onClick={() => captureSingleScreenshot(roundsAreaRef, `Tabelle_${Date.now()}.png`, "Verlauf", "full-table-export")} className="bg-slate-700 text-white font-black py-5 rounded-2xl shadow-xl text-xs flex flex-col items-center justify-center space-y-1">
                  <i className="fas fa-table text-lg"></i><span>Tabelle-Foto</span>
                </button>
              </div>
      ...
      <footer className={`fixed bottom-0 left-0 right-0 py-4 text-center text-[10px] font-black uppercase tracking-widest bg-gray-900/50 backdrop-blur-sm transition-opacity duration-500 ${showModeFooter ? 'opacity-40' : 'opacity-0'}`}>
      ...
      {showStats && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/90 backdrop-blur-2xl">
          <div className={`rounded-3xl p-6 md:p-8 max-w-3xl w-full shadow-2xl border animate-in zoom-in duration-300 flex flex-col ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <h3 className="text-3xl font-black mb-8 text-center uppercase tracking-tighter" style={{ color: BRAND_COLOR }}>Präzision</h3>
            ...
            <div className="grid grid-cols-2 gap-3 mt-10">
                <button onClick={() => captureSingleScreenshot(statsAreaRef, `Stats_${Date.now()}.png`, "Präzision", "stats-export")} className="bg-blue-600 text-white font-black py-4 rounded-xl shadow-lg flex items-center justify-center space-x-2"><i className="fas fa-camera"></i><span>Export</span></button>
      ...
      {showRules && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl animate-in fade-in">
          <div className={`rounded-3xl p-6 md:p-8 max-w-2xl w-full max-h-[90vh] shadow-2xl border flex flex-col ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <h3 className="text-3xl font-black mb-6 text-center uppercase tracking-tighter" style={{ color: BRAND_COLOR }}>Regelwerk</h3>
            ...
            <button onClick={() => setShowRules(false)} className="w-full mt-6 text-white font-black py-4 rounded-xl shadow-lg transition-colors uppercase" style={{ backgroundColor: BRAND_COLOR }}>Alles klar</button>
          </div>
        </div>
      )}
      ...
      {showAutoTargetModal && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/80 backdrop-blur-lg">
          <div className={`rounded-3xl p-8 max-w-md w-full shadow-2xl border-4 border-amber-500 text-center ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <i className="fas fa-magic text-4xl text-amber-500 mb-6"></i>
            <h3 className="text-2xl font-black mb-4 uppercase text-amber-500">Auto-Ziel</h3>
            <p className="opacity-80 mb-8 leading-relaxed text-sm">Die Differenz beträgt <b>{showAutoTargetModal.diff}g</b>. Das Ziel wird automatisch auf <b>{showAutoTargetModal.target}g</b> korrigiert.</p>
            <button onClick={() => { handleTargetWeightConfirm(showAutoTargetModal.target); setShowAutoTargetModal(null); }} className="w-full bg-amber-500 text-white font-black py-4 rounded-xl shadow-lg uppercase">OK</button>
          </div>
        </div>
      )}
      ...
      {disqualifiedNotice && (
        <div className="fixed inset-0 z-[310] flex items-center justify-center p-4 bg-red-600/95 backdrop-blur-md">
          <div className="rounded-3xl p-10 max-w-sm w-full shadow-2xl border-4 border-white text-center bg-white text-red-600">
            <i className="fas fa-user-xmark text-6xl mb-6"></i>
            <h3 className="text-3xl font-black mb-6 uppercase tracking-tighter italic">Ausgeschieden</h3>
            <div className="space-y-4 mb-8">
              {disqualifiedNotice.map((p, i) => (
                <div key={i} className="border-b border-red-100 pb-2 last:border-0">
                  ...
                  <p className="text-xs font-bold opacity-70">Abstand > 50g!</p>
                </div>
              ))}
            </div>
            <button onClick={() => setDisqualifiedNotice(null)} className="w-full bg-red-600 text-white font-black py-4 rounded-2xl shadow-lg uppercase">Bestätigen</button>
          </div>
        </div>
      )}
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-lg">
          <div className={`rounded-3xl p-8 max-w-sm w-full shadow-2xl border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            ...
            <div className="flex flex-col space-y-3">
              <button onClick={resetToStart} className="w-full bg-red-600 text-white font-bold py-4 rounded-xl shadow-lg uppercase">Ja, Beenden</button>
